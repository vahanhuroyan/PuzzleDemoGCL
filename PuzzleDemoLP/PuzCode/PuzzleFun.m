% TAKE APART AN IMAGE
function [pieceMat,allPiece, pieceInfo, im2] = PuzzleFun(im,PP, wide_p,high_p,varargin)
...
[add_imnoise,std,sample] = getPrmDflt(varargin,{'add_imnoise',0,'std',0,'sample',0}, 1);
...
if(add_imnoise)
%    im2 = imnoise(im2,'gaussian',0,sqrt(std));
   im2 = im2 + uint16(normrnd(0,std,[size(im2,1),size(im2,2),size(im2,3)]));   
end
