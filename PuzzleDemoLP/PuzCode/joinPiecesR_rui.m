function [B,R,S1,S2] = joinPiecesR_rui(B1,B2,R1,R2,P1,P2,How,normSCO,ST)
...
%%% CHECK FOR OVERLAP! 
S1 = sum(B(:)>0)==(sum(B1(:)>0)+sum(B2(:)>0));
% Maybe there is a faster way to check for overlap? 

%% check if it is consistent for other induced match as well
neigh1 = get_neigh_from_block(B1,R1n);
neigh2 = get_neigh_from_block(B2,R2n);
neigh = get_neigh_from_block(B,R);

msk1 = ismember(neigh(:,1:2),neigh1(:,1:2),'rows');
msk2 = ismember(neigh(:,1:2),neigh2(:,1:2),'rows');
msk = msk1 | msk2;

neigh = neigh(~msk,:);

mat = [1 13 9 5;  8 4 16 12; 11 7 3 15; 14 10 6 2;
       2 14 10 6; 5 1 13 9;  12 8 4 16; 15 11 7 3;
       3 15 11 7; 6 2 10 14;  9 5 1 13; 16 12 8 4;
       4 16 12 8; 7 3 15 11; 14 6 2 10; 13  9 5 1];
...
for i = 1:size(neigh,1)
    ...
    r2 = neigh(i,4);
    How = neigh(i,5);
    if(normSCO(p1,p2,mat((How-1)*4+r1,r2)) > 10*ST)
       S2 = 0;
    end
end