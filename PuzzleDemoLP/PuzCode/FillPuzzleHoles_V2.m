...
function [newBlock,newRotBlock] = FillPuzzleHoles_V2(Block, Rot, SCO, nr,nc,rotFlag,varargin)
    ...
    if(numel(bb)>0)
        ...
        if(numel(neighbors>0))
            ...
            for ii = 1:1:numel(bb)
                ...
                choicerot = jjj;
                nei = nei_ids(kkk);                
%                 if(rotFlag==0 && jjj ~= nei_code(kkk))
%                     continue;
%                 end
%                 if(rotFlag==1 && mod((jjj-1),4)+1 ~= nei_code(kkk))
%                     continue;
%                 end
