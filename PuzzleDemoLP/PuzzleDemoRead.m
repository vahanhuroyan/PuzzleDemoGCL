                ...
                if(noise.add_imnoise)   %% noise experiment
                    for noise_level = noise.noise_list
                        ...
                        for sample = 1:noise.sample
                            fprintf('sample %d \n',sample);
                            %% create new puzzle
                            res = PuzzleMainRead(Parameters,imname,PP,nc,nr,ScramblePositions,ScrambleRotations,rigid_conncomp,save_path_temp, ...
                    ...
                    [res,max_rank_score] = PuzzleMainRead(Parameters,imname,PP,nc,nr,ScramblePositions,ScrambleRotations,rigid_conncomp,save_path, ...
                        'buddy_check',buddy_check,'loop_check',loop_check,'loop_check2',loop_check2, ...
                        'new_data',0,'speed_up',speed_up,'ranking_method',ranking_method);
                    scores = res.score;
                    if(res.score(1) == 1)
