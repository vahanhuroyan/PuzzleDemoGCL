...
function boundary = find_boundary(BlocksInfo,info_all,info_keep,info_del_track,nr,nc)

info_all = unique(info_all,'rows');
info_keep = unique(info_keep,'rows');
info_del_track = unique(info_del_track,'rows');
...
max_block_dimen = block_dimen_list(ind,:);
