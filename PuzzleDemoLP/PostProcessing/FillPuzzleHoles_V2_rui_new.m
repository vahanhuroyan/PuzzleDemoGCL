...
function [newBlock,newRotBlock] = FillPuzzleHoles_V2_rui_new(Block, Rot, SCO, nr,nc,rotFlag,show,varargin)

[do_replicate,specified_choices,nnfirst,distfirst] = getPrmDflt(varargin,{'do_replicate',0,'specified_choices',[], ... 
                                                               'nnfirst',0,'distfirst',1}, 1);
...
%             scoreMat = getAllScoresForCands_rui_new(SCO, choices, nei_ids(ii), nei_rot(ii), nei_code(ii), rotFlag, nr*nc);
...
while gogo
    ...
    if(numel(ratio_all)>0 && size(neighScores_all,1)-del_num>0)
        ...
        if(size(ratio_all,1) == 77)
           ttt = 1; 
        end
        
        if(distfirst)
            ind_mask = find(nn_sorted == max(nn_sorted));
            dist_all_mask = dist_all(ind_mask,:);
            [aa,phlist] = sort(dist_all_mask);
            phlist = ind_mask(phlist);         
        elseif(nnfirst)
            ind_mask = find(nn_sorted == max(nn_sorted));
            ratio_all_mask = ratio_all(ind_mask,:);
            [aa,phlist] = sort(ratio_all_mask);
            phlist = ind_mask(phlist);
        else
...
%                         scoreMat = getAllScoresForCands_rui_new(SCO, choices, nei_ids(ii), nei_rot(ii), nei_code(ii), rotFlag, nr*nc);
                        if(all(size(ind))) %% this whole is already caculated, just add the cost of newly filled piece
                            ...
                            nn_sorted(addInd) = nn_sorted(addInd) + 1;
                        else
